...
def generate_launch_description():
    ...
    # Add timed actions to ensure proper sequencing
    ld.add_action(TimerAction(period=2.0, actions=[nodes[0]]))  # RSP
    ld.add_action(TimerAction(period=5.0, actions=[nodes[1]]))  # Gazebo
    ld.add_action(TimerAction(period=15.0, actions=[nodes[2]])) # Navigation
    ld.add_action(TimerAction(period=20.0, actions=[nodes[3]])) # Map Environment
