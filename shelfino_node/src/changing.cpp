...
void ShelfinoHWNode::odom_timer_callback()
{
  ...
  // Try to get the transform, but don't fail if it's not available
  try {
    ...
    RCLCPP_DEBUG(
      this->get_logger(), "Could not transform %s to %s: %s. Using default odometry.",
      origin_frame_.c_str(), target_frame_.c_str(), ex.what());
    
    // Instead of returning, we'll use default odometry data
    nav_msgs::msg::Odometry odom_msg;
    odom_msg.header.stamp = this->now();
    odom_msg.header.frame_id = origin_frame_;
    odom_msg.child_frame_id = this->ns + "/base_link";
    
    // Publish identity transform since we don't have T265 data
