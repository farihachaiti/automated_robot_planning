...
def get_borders_points(yaml_file):
    try:
        if "hexagon" in yaml_file['/**']['ros__parameters']['map']:
            return gen_hex_points(float(yaml_file['/**']['ros__parameters']['dx']))
        elif "rectangle" in yaml_file['/**']['ros__parameters']['map']:
...
def spawn_borders(context):
    ...
    if "hexagon" in map_type:
        ...
        with open(os.path.join(context.launch_configurations['gazebo_models_path'], 'hexagon_world', 'model.sdf'), 'r') as file:
