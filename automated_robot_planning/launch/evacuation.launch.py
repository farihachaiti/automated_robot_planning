...
from launch.actions import OpaqueFunction, DeclareLaunchArgument, IncludeLaunchDescription, GroupAction, ExecuteProcess, TimerAction, SetEnvironmentVariable
...
def generate_launch_description():
    ...
    robot_desc = LaunchConfiguration('robot_description', default=os.path.join(shelfino_desc_pkg, 'models', 'shelfino_v1.xacro'))
    ...
    def launch_evacuation_nodes(context):
        ...
        for shelfino_id in range(int(context.launch_configurations['n_shelfini'])):
            ...
            node_name = "shelfino_robot_" + str(shelfino_id)
            ...
            robot_controller_node = Node(
                ...
                remappings=[
                            ('/follow_path', f'/{shelfino_name}/follow_path'),
                        ],
            ...
            path_planner_node = Node(
                ...
                parameters=[{'robot_description' : robot_desc},
                            {'robot_name': shelfino_name}],
                remappings=[
                            ("/obstacles", f'/{shelfino_name}/obstacles'),
                            ("/gate_position", f'/{shelfino_name}/gate_position'),
                            ("/map_borders", f'/{shelfino_name}/map_borders'),
                            ("/tf", f'/{shelfino_name}/tf_publisher'),
                            (f'/{shelfino_name}/global_costmap/costmap', f'/{shelfino_name}/global_costmap/g_costmap_subscriber'),
                            (f'/{shelfino_name}/local_costmap/costmap', f'/{shelfino_name}/local_costmap/l_costmap_subscriber'),
                            (f'/{shelfino_name}/joint_states', f'/{shelfino_name}/joint_states_publisher'),
                        ],
                emulate_tty=True,  # Keeps color in the terminal
            )
            ...
            obstacle_detector_node = Node(
                ...
                remappings=[(f'/{shelfino_name}/global_costmap/costmap', f'/{shelfino_name}/global_costmap/g_costmap_publisher'),
                            (f'/{shelfino_name}/local_costmap/costmap', f'/{shelfino_name}/local_costmap/l_costmap_publisher'),
                        ],
                emulate_tty=True,  # Keeps color in the terminal
            )

            shelfino_robot_node = Node(
                package='automated_robot_planning',
                executable='shelfino_robot.py',
                name='shelfino_robot',
                namespace=f'/{shelfino_name}',
                output='screen',
                parameters=[{'robot_description' : robot_desc},
                            {'robot_name': shelfino_name}],
                remappings=[
                            ("/tf", f'/{shelfino_name}/tf_subscriber'),
                            (f'/{shelfino_name}/joint_states', f'/{shelfino_name}/joint_states_subscriber'),
                        ],
                emulate_tty=True,  # Keeps color in the terminal
                )
            nodes.append(obstacle_detector_node)
            nodes.append(path_planner_node)
            nodes.append(shelfino_robot_node)            
    ...
    def launch_navigation(context):
        ...
        for shelfino_id in range(int(context.launch_configurations['n_shelfini'])):
            ...
            model_output = os.path.join(shelfino_desc_pkg, 'models', 'shelfino') + str(shelfino_id) + ".sdf"
            sapf_node = GroupAction([
                IncludeLaunchDescription(
                    ...
                    # Spawn Shelfino robot directly from Xacro file
                    '''ExecuteProcess(
                        cmd=['ros2', 'run', 'gazebo_ros', 'spawn_entity.py', '-file', robot_desc, '-entity', shelfino_name, '-topic', '/' + shelfino_name + '/robot_description', '-spawn_service', '/spawn_entity', '-output', model_output],
                        output='screen',
                        shell=True,
                    ),'''
                ])
    ...
    ld.add_action(TimerAction(
        period=5.0,
        actions = [OpaqueFunction(function=launch_evacuation_nodes)]
    ))
